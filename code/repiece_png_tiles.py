...
def get_num_horizontal_positions(input_folder):
	...
	for image_path in image_paths:
		x_increment = int(image_path.replace('__', '_').split('/')[-1].split('.')[0].split('_')[1])
...
def get_num_vertical_positions(input_folder):
	...
	image_paths = get_image_paths(input_folder)
	for image_path in image_paths:
		x_increment = int(image_path.replace('__', '_').split('/')[-1].split('.')[0].split('_')[2])
...
def output_repieced_image(input_folder, output_image_path):
	...
	for image_path in image_paths:
		...
		x_increment = int(image_path.replace('__', '_').split('/')[-1].split('.')[0].split('_')[1])
		y_increment = int(image_path.replace('__', '_').split('/')[-1].split('.')[0].split('_')[2])
